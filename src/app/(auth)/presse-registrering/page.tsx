    ...
    return (
      <div className="min-h-screen bg-theme flex items-center justify-center px-4">
        <div className="w-full max-w-sm text-center">
          ...
          <h1 className="text-xl font-bold text-theme mb-2">Søknad sendt!</h1>
    ...
    <div className="min-h-screen bg-theme flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-theme">Pressetilgang</h1>
          <p className="text-theme-secondary text-sm mt-1">Søk om tilgang til presseinformasjon fra Brannloggen</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-theme-card border border-theme-input rounded-lg text-theme focus:outline-none focus:border-cyan-500"
          ...
          <div>
            <label className="block text-sm text-theme-secondary mb-1">Mediehus / redaksjon *</label>
            {medierLoading ? (
              <div className="w-full px-4 py-3 bg-theme-card border border-theme rounded-lg text-theme-muted text-sm">Laster medier...</div>
            ) : (
              <>
                <input
                  type="text"
                  ...
                  className="w-full px-4 py-3 bg-theme-card border border-theme-input rounded-lg text-theme focus:outline-none focus:border-cyan-500 mb-2"
                  placeholder="Søk eller skriv inn mediehus..."
                />
                {medierSok && !mediumId && (
                  <div className="max-h-40 overflow-y-auto bg-theme-card border border-theme rounded-lg">
                    {filteredMedier.slice(0, 8).map(m => (
                      <button
                        ...
                        className="w-full text-left px-4 py-2.5 text-sm text-theme hover:bg-theme-card-hover transition-colors"
                      >
                        {m.navn}
                        <span className="text-xs text-theme-muted ml-2">{m.type}</span>
                      </button>
                    ))}
                    {filteredMedier.length === 0 && (
                      <div className="px-4 py-2.5 text-sm text-theme-secondary">
                ...
                {mediumId && (
                  <div className="flex items-center gap-2 px-3 py-2 bg-cyan-500/10 border border-cyan-500/20 rounded-lg">
                    ...
                    <button type="button" onClick={() => { setMediumId(''); setMedierSok(''); setMediehus('') }} className="text-xs text-theme-secondary hover:text-theme ml-auto">Endre</button>
                  </div>
                )}
              </>
            )}
          ...
          <div>
            <label className="block text-sm text-theme-secondary mb-1">Telefon</label>
            <input
              type="tel"
              value={telefon}
        ...
        <div className="mt-6 text-center space-y-2">
          <a href="/login" className="block text-sm text-theme-secondary hover:text-theme">
